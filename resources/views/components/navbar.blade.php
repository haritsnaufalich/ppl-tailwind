  ...
  @if (request()->is('dashboard'))
    <div class="flex items-center">
      <form action="/logout" method="post">
        @csrf
        <button class="flex font-medium py-2 px-4 items-center rounded-xl bg-[#eceeed] text-[#202221]"><a
            href="/auth/login">Sign Out</a></button>
      </form>
    </div>
  @else
    @auth
      <div class="flex items-center hover:cursor-pointer relative" id="toggle">
        <span>Welcome,
          Harits Naufal <i class="fa-solid fa-caret-down ml-3"></i></span>
        <div class="absolute flex-col items-end p-3 gap-3 bg-[#eceeed] rounded-xl top-[60px] right-0 w-3/4 hidden"
          id="dropdown-box">
          <a class="text-[#202221] hover:bg-[#202221] hover:p-1.5 hover:rounded-xl hover:text-[#eceeed] hover:cursor-pointer hover:m-[-6px]"
            href="/dashboard">Dashboard<i class="fa-solid fa-lock ml-3"></i></a>
          <hr class="border-[#202221] w-full">
          <form action="/logout" method="post">
            @csrf
            <button
              class="text-[#202221] hover:bg-[#202221] hover:p-1.5 hover:rounded-xl hover:text-[#eceeed] hover:cursor-pointer hover:m-[-6px]">Sign
