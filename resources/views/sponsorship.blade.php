...
@section('content')
  <div class="flex flex-col space-y-6">
    <div class="flex items-center">
      <h2 class="text-xl font-semibold text-[#eceeed]">Sponsorship</h2>
    </div>
      ...
      <div class="flex flex-col gap-6 p-6 rounded-xl bg-[#202221]">
            ...
            <label for="sponsorship_type_id">
              <select name="sponsorship_type_id" id="sponsorship_type_id"
                ...
                @foreach ($sponsorship_types as $sponsorship_type)
                  <option class="bg-[#202221]" value="{{ $sponsorship_type->id }}">{{ $sponsorship_type->name }}</option>
                @endforeach
              </select>
            </label>
          </div>
        ...
        <div class="grid grid-cols-1">
          <textarea name="description" id="description" rows="20" class="py-3 px-6 bg-[#5B625F] rounded-xl w-full"
            placeholder="Sponsorship Description"></textarea>
        </div>
